    ...
    <div className="flex justify-center items-center ">
      ...
      {isOpen && (
        <div className="fixed z-50 inset-0 flex  items-center justify-center bgBlur" onClick={() => setIsOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="z-50 fixed  bg-slate-800 py-6 rounded-4xl shadow-lg w-[60%] max-lg:w-[90%] h-[80%] max-lg:h-[76%] border-[1px] border-slate-500 flex flex-col space-y-2 justify-between">
            <div className="w-full flex  max-md:flex-col-reverse items-center justify-items-center text-center justify-between border-b-[1px] border-slate-500 max-md:text-xl">
                  {/* Botão para fechar */}
            <h2 className="text-orange-200 text-4xl text-left px-8 py-4 max-md:text-3xl">Sobre a Rinoplastia</h2>
            <button
            ...
            <h2 className="font-semibold mb-2 text-blue-500 text-3xl px-8 max-md:text-2xl">O que é a Rinoplastia?
            </h2>
            <p className="text-gray-100 px-8 py-2 text-lg max-md:text-md">A rinoplastia é uma das cirurgias mais procuradas para a remodelação nasal e pode melhorar tanto a aparência quanto a função respiratória do paciente. O período pós-operatório é fundamental para a obtenção de um bom resultado, e os cuidados adequados garantem não apenas a recuperação, mas também a manutenção das mudanças estéticas planejadas. Este guia foi elaborado para esclarecer as principais orientações de cuidados após uma rinoplastia.
</p>
